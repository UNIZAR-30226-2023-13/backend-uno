...
import {
    ...
    cambiarTablero,
    getAspectosTablerosDesbloqueados,
    getAspectosCartas,
    getAspectosTableros,
    getAspectosCartasDesbloqueados,
...
aspectosRouter.get(
    ...
    async (req: Request, res: Response) => {
        ...
        if (req.session.username) {
            username = req.session.username;
            const aspectos: Aspecto[] = await getAspectosCartasDesbloqueados(
                username
            );
            res.status(200);
            res.json(aspectos);
...
aspectosRouter.get(
    "/tableros/desbloqueados",
    async (req: Request, res: Response) => {
        let username = "";
        if (req.session.username) {
            username = req.session.username;
            const aspectos: Aspecto[] = await getAspectosTablerosDesbloqueados(
                username
            );
            res.status(200);
            res.json(aspectos);
